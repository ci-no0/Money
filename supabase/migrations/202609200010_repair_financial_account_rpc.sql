-- Repair the account RPC for databases where migration 002 ran partially
-- or PostgREST has not cached the function definition.
...
begin
    ...
    if trim(coalesce(account_name, '')) = '' then
        raise exception 'Account name is required';
    end if;

    if requested_starting_balance is null or requested_starting_balance < 0 then
        raise exception 'Starting balance cannot be negative';
    end if;

    normalized_currency := upper(trim(coalesce(requested_currency, 'PHP')));
    ...
    values (requested_workspace, 'Opening Balance Equity', 'equity', normalized_currency)
